...
**Structured event logging library for Bash** — dual log model (application + audit),
8 output modules, 23-type event taxonomy, syslog/JSON/CEF/GELF/ELK output, severity
filtering, zero external dependencies.

> (C) 2002-2026, [R-fx Networks](https://www.rfxn.com) <proj@rfxn.com>
...
```bash
source /opt/myapp/lib/elog_lib.sh           # Load the library
ELOG_APP="myapp"                            # Set application name
ELOG_LOG_DIR="/var/log/myapp"               # Set log directory
elog_init                                   # Initialize log environment
elog_output_enable "stdout"                 # Enable terminal output (not automatic)

elog info "application started"             # Log an operational message
elog warn "{auth} failed login attempt"     # Log with tag extraction
elog_event "config_loaded" "info" \         # Log a structured audit event
...
elog info "works without init"              # Auto-enables output modules on first use
```
...
elog_lib is a shared Bash library for structured event logging with a dual log
model (application + audit), output module registry, event taxonomy, and
classic/JSON output formats. Source it into your script, set environment
variables, and call `elog` to log — no dependencies, no subprocesses.
...
```bash
cp files/elog_lib.sh /opt/myapp/lib/        # Copy library to project
chown root:root /opt/myapp/lib/elog_lib.sh  # Set ownership
chmod 640 /opt/myapp/lib/elog_lib.sh        # Set permissions
```
...
elog_lib is configured entirely through environment variables. Set these
before calling `elog_init()` or the first `elog()` call.
...
**Development** — verbose output with debug messages on stdout:
...
**Production daemon** — file and audit logging with syslog, no terminal output:
...
**SIEM integration** — all SIEM modules enabled, JSON format:
...
**Monitoring-only (Graylog)** — centralized logging, no local files:
...
elog_lib exposes public functions for operational logging (`elog`), structured
event logging (`elog_event`), output module management, and log rotation.
...
`debug` level outputs to stdout only (bare text), gated by `ELOG_VERBOSE=1`,
and never writes to files. `info` and above are formatted and routed through
dispatch to enabled modules. Empty messages produce no output.
...
- `event_type` — event type name (see Event Taxonomy in section 6)
...
- `format` — `classic`, `json`, `cef`, `gelf`, or `elk`
- `source` — `all`, `elog`, or `event`
...
### 4.2 When to Use elog() vs elog_event()
...
elog_lib functions use shell return codes as exit codes. Logging functions are
designed to never cause caller failure under normal operation.

| Function | Code | Meaning |
|----------|------|---------|
| `elog_init()` | 0 | Log environment initialized successfully |
| `elog_init()` | 1 | Directory creation failed |
| `elog()` | 0 | Always returns 0 (logging must never cause caller failure) |
| `elog_event()` | 0 | Event logged or suppressed by severity filter |
| `elog_event()` | 1 | Empty event type (stderr warning emitted) |
| `elog_output_register()` | 0 | Module registered successfully |
| `elog_output_register()` | 1 | Empty name/handler or duplicate name |
| `elog_output_enable()` | 0 | Module enabled |
| `elog_output_enable()` | 1 | Module not registered |
| `elog_output_disable()` | 0 | Module disabled |
| `elog_output_disable()` | 1 | Module not registered |
| `elog_output_enabled()` | 0 | Module is enabled |
| `elog_output_enabled()` | 1 | Module not registered or disabled |

## 5. Architecture

elog_lib implements a dual log model with a pluggable output module registry
and source-based dispatch filtering.
...
23 canonical event types across 7 categories define the structured event
vocabulary. The taxonomy is guidance — `elog_event()` does not enforce type
validation; consumers may pass any string.
...
elog_lib includes built-in output modules for four SIEM platforms. Each module
receives only `elog_event()` output (except syslog_udp which receives both).

### 7.1 CEF Output (ArcSight)
...
# Output: CEF:0|R-fx Networks|myapp|1.0.4|block_added|blocked host|5|src=203.0.113.42 reason=SSH
```

CEF severity mapping: debug=1, info=3, warn=5, error=7, critical=10.

### 7.2 Syslog UDP (RFC 5424/3164)
...
# Output: {"version":"1.1","host":"srv1","short_message":"blocked host",...}
```
...
### 7.4 ELK JSON Output (Elasticsearch)
...
**Step 1: Copy the library** into your project's internal library directory:
...
**Step 2: Source the library** early in your startup chain, after path
discovery but before any logging calls:
...
**Step 3: Map project config** to ELOG_* variables from your project's own
config names. This mapping belongs in `internals.conf` or config-loading code:
...
**Step 4: Call elog_init()** once during startup, after all ELOG_* variables
are set:
...
**Step 5: Enable stdout (CRITICAL).** `elog_init()` does NOT enable stdout
output. This is intentional for daemons that should not write to the terminal
after initialization. If your project needs stdout output:
...
Worked examples demonstrating common deployment patterns.

### 9.1 Firewall Event Logging (APF Pattern)
...
### 9.2 Intrusion Detection (BFD Pattern)
...
### 9.3 Malware Scanner (LMD Pattern)
...
elog_output_enable "cef"
elog_event "block_added" "warn" "blocked host" "src=203.0.113.42"
# Output: CEF:0|R-fx Networks|myapp|1.0.4|block_added|blocked host|5|src=203.0.113.42
...
# Sends: {"@timestamp":"...","event.action":"scan_completed",...}
```

## 10. Troubleshooting

Common issues and their resolutions when integrating or operating elog_lib.

### 10.1 No Stdout Output
...
### 10.3 SIEM Messages Not Arriving
...
### 10.5 Log File Growing Too Large
...
Tests use the [batsman](https://github.com/rfxn/batsman) framework across
11 BATS files:
...
Tests run inside Docker containers via batsman. CI runs lint + full matrix
...
- **Issues:** [GitHub Issues](https://github.com/rfxn/elog_lib/issues)
- **Email:** proj@rfxn.com
